...
> **定位**：XRK-AGT 是**融合智能体业务逻辑的通用后端**；本页描述从 `node dist/app.js` 到 AgentRuntime 在线的引导链。架构边界见 [底层架构设计](底层架构设计.md)。
...
```mermaid
flowchart TD
  CLI["pnpm start / node dist/app.js"] --> App["dist/app.js"]
  App --> Boot["dist/src/utils/bootstrap.js"]
  ...
  Boot --> Start["import dist/start.js"]
  Start --> Menu["交互菜单 / PM2 / server 子进程"]
  Start --> AgentRuntime["dist/src/agent-runtime.js · AgentRuntime.run"]
...
| 阶段 | 源码 | 运行产物 | 职责 |
|------|------|----------|------|
| 入口 | `app.ts` | `dist/app.js` | `new Bootstrap().run()` |
| 引导 | `src/utils/bootstrap.ts` | `dist/src/utils/bootstrap.js` | 环境、依赖、动态 imports |
| 菜单 / 进程 | `start.ts` | `dist/start.js` | 端口选择、Playwright 浏览器、PM2、Ctrl+C 语义 |
| 运行时 | `src/agent-runtime.ts` | `dist/src/agent-runtime.js` | 服务、加载器、全局 `AgentRuntime` |

引导日志：`logs/bootstrap.log`（`src/utils/simple-logger.ts`）。

**配置与代码变更**：YAML、插件、工作流、模板均无热重载；修改后需 **重启进程**（改 TypeScript 源码后须先 `pnpm build`）。见 [ADR-0004](adr/0004-typescript-dist-no-hot-reload.md)。

---

## Bootstrap 步骤

实现位于 `src/utils/bootstrap.ts`（`app.ts` / `dist/app.js` 仅一行调用）。
...
2. 源码 `core/*` 含 `package.json` 的子包各自 `pnpm install`（勿对空的 `dist/core/*/node_modules` 误装）。
...
| `XRK_FAST_START=1` | 加快启动（跳过部分检查） |
| `XRK_SERVER_PORT` | 默认服务端口 |
| `XRK_FROM_APP=1` | 由 `dist/app.js` 拉起时由引导设置 |
...
```bash
pnpm build                 # tsc + 资源拷贝 → dist/
pnpm start                 # build && node dist/app.js
node dist/app.js server 8080
node dist/start.js server 8080   # 会重定向到 dist/app.js
pnpm debug                 # build && node dist/debug.js
```
