...
> **TypeScript**：生产主路径为 `pnpm build` → `node dist/app.js`（[ADR-0004](adr/0004-typescript-dist-no-hot-reload.md)）。Core 源码 `.ts` 经 tsc 进 `dist/core/`；同名优先以构建产物为准。见 [node-26-runtime.md](node-26-runtime.md)、[startup.md](startup.md)。
