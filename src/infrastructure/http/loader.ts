...
class HttpApiLoader {
  ...
  async load() {
    ...
    await FileLoader.forEachBatch(allFiles, LOADER_BATCH_SIZE, (file: string) => this.loadApi(file))
    ...
    RuntimeUtil.makeLog(
      'info',
      `API模块加载完成: ${this.apis.size}个, 耗时${Date.now() - startTime}ms`,
      'HttpApiLoader'
    )
    return this.apis
  }

  async getApiKey(filePath: string) {
  ...
  async loadApi(filePath: string) {
    try {
      ...
      RuntimeUtil.makeLog('error', `加载API失败: ${filePath} - ${detail}`, 'HttpApiLoader', true)
      return false
    }
  }

  async unloadApi(key: string) {
  ...
  _removeWsHandlersByOwner(ownerKey: string) {
  ...
  async _reloadFromFile(key: string, filePath: string) {
  ...
  async register(app: any, bot: any) {
    ...
    RuntimeUtil.makeLog(
      'info',
      `API路由注册完成: ${enabledCount}个模块, ${totalRoutes}个路由, ${totalWS}个WebSocket`,
      'HttpApiLoader'
    )
  }

  async changeApi(key: string, filePath: string | null = null) {
    const api = this.apis.get(key)
    ...
    try {
      ...
      RuntimeUtil.makeLog(
        'info',
        `API重载成功: ${this.apis.get(key)?.name ?? key}`,
        'HttpApiLoader'
      )
      return true
    } catch {
      RuntimeUtil.makeLog('error', `API重载失败: ${key}`, 'HttpApiLoader', true)
      return false
    }
  }

  async _findApiFile(key: string) {
    const apiDirs = await paths.getCoreSubDirs('http')
    for (const apiDir of apiDirs) {
      const files = await FileLoader.readFiles(apiDir, { recursive: true })
      const file = files.find((f: string) => resolveQualifiedCoreModuleKey(f, [apiDir], 'http') === key)
      if (file) return file
    }
    return null
  }
