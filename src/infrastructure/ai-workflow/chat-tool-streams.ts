...
export function isRemoteMcpStreamName(name: unknown) {
  return String(name ?? '').startsWith('remote-mcp.')
}

/** 可 merge 的实体流 vs 仅工具面名字（remote-mcp.*） */
export function partitionToolStreamNames(names: unknown) {
...
export function expandChatToolWorkflowWhitelist(baseNames: unknown) {
