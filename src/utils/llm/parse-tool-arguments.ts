...
function onlyRawString(obj: Record<string, unknown>): string | null {
...
export type ParseToolArgsOk = { ok: true; args: Record<string, unknown> };
export type ParseToolArgsFail = {
  ok: false;
...
export function parseToolCallArguments(rawInput: unknown): ParseToolArgsOk | ParseToolArgsFail {
  if (rawInput == null || rawInput === '') {
    return { ok: true, args: {} };
  }

  let cur: unknown = rawInput;
...
export function previewToolCallArguments(rawInput: unknown): string {
...
/** 解析失败时回给模型的统一文案 */
export function toolArgumentsParseHint(error: string): string {
