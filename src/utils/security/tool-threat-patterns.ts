...
export const TOOL_THREAT_PATTERNS: ThreatPattern[] = [
  {
    name: 'rm_rf_root',
...
export function scanTextForThreats(
  text: unknown,
): Array<{ name: string; description: string; risk: RiskLevel }> {
  const s = String(text || '');
  if (!s.trim()) return [];
  const hits: Array<{ name: string; description: string; risk: RiskLevel }> = [];
