...
type GLoggerFn = (...args: unknown[]) => void;
...
const gLogger = (): GLoggerLike | undefined =>
    (globalThis as typeof globalThis & { logger?: GLoggerLike }).logger;
...
function errorDetail(error: unknown): string {
    if (Error.isError(error)) return error.stack || error.message;
    return String(error);
}
...
interface SystemStatus {
    ...
    leak: LeakStatus | null;
    disk: DiskStatus | null;
    network: NetworkStatus | null;
    fileHandles: FileHandlesStatus | null;
}

class SystemMonitor extends EventEmitter {
    static instance: SystemMonitor | null = null;
    ...
    _optimizing = false;
    browserCache: { data: BrowserProcess[]; timestamp: number; ttl: number } = {
        data: [],
        timestamp: 0,
        ttl: 5000,
    };
    ...
    fileHandles = { open: 0, max: 0 };
    /** Redis 客户端可选挂载（未就绪则为 null） */
    redis: unknown = null;
    /** 经 normalizeMonitorConfig 后的监控配置 */
    config: NormalizedMonitorConfig | null = null;
    _lastSystemMemory: SystemMemorySnapshot | null = null;
     ...
     *
     * @param config 原始 `runtimeConfig.monitor`
     */
     ...
     */
    async safeRun(task: () => Promise<void>, label = '系统任务'): Promise<void> {
        try {
            await task();
        } catch (error: unknown) {
            gLogger()?.error?.(`${label}失败: ${errorDetail(error)}`);
        }
    }
    ...
    async checkSystem(): Promise<void> {
        ...
        try {
            ...
            gLogger()?.error?.(`系统检查失败: ${errorMessage(error)}`);
        } finally {
    ...
    async checkMemory(): Promise<MemoryStatus> {
        ...
        const systemMemory: SystemMemorySnapshot = {
            total: mem.total,
            free: mem.free,
            used: mem.used,
     ...
     * 采样本进程 CPU（短窗口差分）。
     */
    async checkCPU(): Promise<CpuStatus | null> {
        ...
        return {
            ...
            warning: cpuPercent > (this.config.cpu?.threshold ?? 90)
        };
    }
     ...
     */
    async checkBrowser(): Promise<BrowserStatus | null> {
        ...
        if (needCleanup) {
            gLogger()?.warn?.(`检测到浏览器进程过多 (${processes.length}/${maxInstances})，执行清理...`);
     ...
     */
    async detectBrowserProcesses(): Promise<BrowserProcess[]> {
     ...
     */
    parseBrowserProcesses(output: string, platform: NodeJS.Platform): BrowserProcess[] {
     ...
     * @returns 成功结束的进程数
     */
    async cleanupBrowsers(processes: BrowserProcess[]): Promise<number> {
        const managed = processes.filter((p) => this._isManagedBrowserCommand(p.command || ''));
        if (managed.length === 0) {
            gLogger()?.debug?.('浏览器实例偏多，但无 AGT 托管进程可清理，已跳过 taskkill');
            return 0;
        }
        ...
        if (cleaned > 0) {
            gLogger()?.info?.(`已清理 ${cleaned} 个托管浏览器进程 (保留最多 ${maxInstances} 个)`);
        }
        return cleaned;
    }
     ...
     * @private
     */
    _isManagedBrowserCommand(commandLine: string): boolean {
     ...
     */
    analyzeStatus(status: SystemStatus): boolean {
        const issues: string[] = [];
        ...
        if (status.disk != null && status.disk.usedPercent > 90) {
            issues.push('disk');
            gLogger()?.warn?.(`磁盘使用率过高: ${status.disk.usedPercent.toFixed(1)}%`);
        }

        if (status.network?.warning) {
            issues.push('network');
            gLogger()?.warn?.(`网络连接数过多: ${status.network.connections}`);
        }

        if (status.fileHandles?.warning) {
            issues.push('fileHandles');
            gLogger()?.warn?.(`文件句柄使用率过高: ${status.fileHandles.usagePercent.toFixed(1)}%`);
        }
     ...
     */
    detectMemoryLeak(): LeakStatus | null {
        ...
        if (this.leakDetection.baseline == null) {
            this.leakDetection.baseline = heapUsed;
            this.leakDetection.lastCheck = now;
            return null;
        }

        const baseline = this.leakDetection.baseline;
        const growth = (heapUsed - baseline) / baseline;
        ...
        if (avgGrowth > this.leakDetection.threshold) {
            ...
            gLogger()?.warn?.(`⚠️ 检测到潜在内存泄漏: 内存增长 ${growthPercent}%`);
            this.emit('leak', {
                growth: avgGrowth,
                baseline,
                current: heapUsed,
     ...
     */
    async checkDisk(): Promise<DiskStatus | null> {
        try {
     ...
     * @private
     */
    _estimateNetworkConnections(): number {
     ...
     * 采样本进程打开文件句柄数 / 上限。
     */
    async checkFileHandles(): Promise<FileHandlesStatus | null> {
        try {
            const platform = process.platform;
     ...
     * @private
     */
    async _getFileHandleLimit(): Promise<number> {
        try {
     ...
     * @private
     */
    async _getLinuxFileHandles(pid: number): Promise<number> {
        try {
     ...
     * @private
     */
    async _getWindowsFileHandles(pid: number): Promise<number> {
        try {
     ...
     * @private
     */
    async _getDarwinFileHandles(pid: number): Promise<number> {
        try {
            const { stdout } = await exec(`lsof -p ${pid} 2>/dev/null | wc -l`);
            return parseInt(stdout.trim()) || 0;
        } catch {
     ...
     */
    async optimizeSystem(status: SystemStatus | null = null): Promise<void> {
        ...
        try {
            ...
            gLogger()?.debug?.('系统优化完成');
        } catch (error: unknown) {
            gLogger()?.error?.(`系统优化失败: ${errorMessage(error)}`);
        } finally {
    ...
    async optimizeMemory(): Promise<void> {
        ...
        if (leakInfo) {
            gLogger()?.warn?.(`内存泄漏检测: 当前 ${(leakInfo.current / 1024 / 1024).toFixed(2)}MB, 基线 ${(leakInfo.baseline / 1024 / 1024).toFixed(2)}MB`);
        }
        ...
        if (freed > 0) {
            gLogger()?.debug?.(`内存优化: 释放 ${(freed / 1024 / 1024).toFixed(2)}MB`);
        }

        const baseline = this.leakDetection.baseline;
        if (baseline != null && afterMem.heapUsed < baseline * 0.9) {
     ...
     */
    async optimizeDisk(): Promise<void> {
        try {
            ...
            gLogger()?.error?.(`磁盘优化失败: ${errorMessage(error)}`);
        }
    }
    ...
    async cleanupTempFiles(): Promise<void> {
        try {
            ...
            if (cleaned > 0) {
                gLogger()?.debug?.(`清理临时文件: ${cleaned} 个，释放 ${(freed / 1024 / 1024).toFixed(2)}MB`);
            }
        } catch (error: unknown) {
            gLogger()?.error?.(`清理临时文件失败: ${errorMessage(error)}`);
        }
    }
    ...
    async cleanupLogFiles(): Promise<void> {
        try {
            ...
            try {
                ...
                if (cleaned > 0) {
                    gLogger()?.debug?.(`清理日志: ${cleaned} 个，释放 ${(freed / 1024 / 1024).toFixed(2)}MB`);
                }
            } catch {
                // 目录不存在跳过
            }
        } catch (error: unknown) {
            gLogger()?.error?.(`清理日志文件失败: ${errorMessage(error)}`);
        }
    }
    ...
    async clearSystemCache(): Promise<void> {
        ...
        try {
            ...
            if (commands.length) gLogger()?.debug?.('系统缓存清理命令已执行');
        } catch {
            // 整段失败忽略
        }
    }
     ...
     */
    async optimizeProcess(): Promise<void> {
        try {
            const platform = process.platform;
            ...
            if (platform === 'linux') {
                if (nice !== 0) {
                    try {
                        (process as NodeJS.Process & { setPriority?: (nice: number) => void }).setPriority?.(nice);
                        gLogger()?.info?.(`  ✓ 已设置进程优先级 (nice: ${nice})`);
                    } catch {
                        // 权限不足跳过
                    }
                }
                ...
                try {
                    ...
                    gLogger()?.info?.(`  ✓ 已设置进程优先级 (${priority})`);
                } catch {
                    // wmic 失败跳过
                }
            }
    ...
    getSystemMemory(): SystemMemorySnapshot {
